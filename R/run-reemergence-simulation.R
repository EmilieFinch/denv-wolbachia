...
# load model
denv_mod <- odin2::odin(here("R/denv-serotype-model.R"), debug = TRUE, skip_cache = TRUE) 


r0s <- seq(5, 0.5, by = -0.5)
inhibs <- seq(0.8,0, by = -0.1)
for(r0 in r0s){
  for(inhib_level in inhibs){ 
...
cat(paste0("Running simulations for r0: ", r0, " and inhibition level: ", inhib_level))
sim_raw <- dust_system_simulate(denv_sys, t, index_state = idx)

## Wrangle and save output
cat(paste0("Saving output for r0: ", r0, " and inhibition level: ", inhib_level))
...
quantiles_out <- sim_out |>  
  group_by(state, name, level, time, inhib_level, r0, serotype_varying) |> 
...
qsave(quantiles_out, here(output_path, paste0("quantile-out_r0-",r0, "_inhib-level-", inhib_level,"_serotype-varying-", serotype_varying, ".qs")))
...
if (file.exists(here(output_path, "time-to-emergence.csv"))) {
  write_csv(emergence_out, here(output_path, paste0("time-to-emergence.csv")),append = TRUE)
} else {
  write_csv(emergence_out, here(output_path, paste0("time-to-emergence.csv")))
}
